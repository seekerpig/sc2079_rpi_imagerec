...
class MultiProcess:
    ...
    def receiveFromAndroid(self):
        while True:
            try:
                m = "MANUAL|FW10" # MANUAL|FR00 MANUAL|FW70 MANUAL|BR00 MANUAL|BW20 MANUAL|FIN
                n = "TASK1|[{'x':6,'y':2,'d':4}, {'x':4,'y':2,'d':0}, {'x':5,'y':2,'d':2}]"
                rawMessage = m
                ...
                if(rawMessage):
                    ...
                    if rawMessage.startswith(Protocol.Android.TASK1):
                        ...
                        if (len(messageList) > 1):
                            ...
                            print("message added to algo queue")
                            self.unpause.set()
                            self.sendToAlgo()
                        else:
    ...
    def receiveFromAlgo(self):
        while True: 
            b = "TASK1|['FW10', 'FR00', 'FW70', 'BR00', 'BW20', 'SNAP', 'FW20', 'FIN']|[{'x':1,'y':1,'d':0, 's':0}, {'x':4,'y':2,'d':2. 's':0}, {'x':5,'y':2,'d':2, 's':1}]"
            try:
                 rawMessage = b
        ...
        while True:
            try:
               if not self.toAlgoQueue.empty():
                   ...
                   #self.receiveFromAlgo.send(message)
        ...
        while True:
            try:
                ...
                if not self.toSTMQueue.empty():
                    ...
                    # Instructions for STM if the message is part of movements/ new changes update!
